...
export default function Sidebar({ isOpen = true, onClose, isMobile = false }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`${isMobile ? "fixed left-0 top-0 bottom-0 z-50 transition-transform duration-300 overflow-y-auto" : ""} w-[296px] flex-shrink-0 border-r px-4 py-6 flex flex-col gap-5`}
        style={{
      ...
      >
        ...
        <Link href="/" onClick={handleLinkClick} className="block hover:opacity-90 transition-opacity" style={{ textDecoration: "none" }}>
          <div className="w-[260px] h-[260px] rounded-full mb-4 p-[4px]" style={{ background: "#1f9cf0" }}>
            <div className="w-full h-full rounded-full p-[12px]" style={{ background: "var(--bg-canvas)" }}>
              <div className="w-full h-full rounded-full overflow-hidden">
                <Image src="/avatar.png" alt="Robert Jean Pierre" width={240} height={240} className="w-full h-full object-cover" priority />
              </div>
            </div>
          </div>
          <div className="text-[26px] font-semibold leading-tight" style={{ color: "var(--fg-default)" }}>Robert Jean Pierre</div>
          <div className="text-[20px] font-light mt-0.5" style={{ color: "var(--fg-muted)" }}>rpmjp</div>
          <p className="text-[16px] mt-4 leading-relaxed" style={{ color: "var(--fg-default)" }}>ML engineer building production systems. NJIT CS &apos;26.</p>
        </Link>

        <div className="flex flex-col gap-2 text-[14px]">
          <SidebarItem icon={<MapPin size={16} />} text="North Brunswick, NJ" />
          <SidebarItem icon={<GraduationCap size={16} />} text="NJIT · Rutgers" />
          <SidebarItem icon={<Briefcase size={16} />} text="Previously Verizon" />
          <SidebarLink icon={<Mail size={16} />} text="robertjp@live.com" href="mailto:robertjp@live.com" />
        ...
        <div className="text-[12px] px-2 py-1 rounded-md inline-flex items-center gap-1.5 self-start" style={{ background: "var(--success-bg)", color: "var(--success-fg)", border: "1px solid var(--success-fg)" }}>
